...
def alpha_14(open, volume, returns):
...
def alpha_22(high, close, volume):
...
def alpha_25(high, close, vwap, adv20, returns):
    return rank(((((-1 * returns) * adv20) * vwap) * (high - close)))


def alpha_26(high, volume):
...
def alpha_41(low, high, vwap):
    return ((high * low) ^ 0.5) - vwap


def alpha_42(close, vwap):
    return rank((vwap - close)) / rank((vwap + close))


def alpha_43(close, volume, adv20):
...
def alpha_54(open, low, high, close):
    return (-1 * ((low - close) * (open ^ 5))) / ((low - high) * (close ^ 5))


def alpha_55(low, high, close, volume):
...
def alpha_59(volume, vwap, indclass):
    return -1 * ts_rank(
        decay_linear(
            correlation(
...
def alpha_64(open, low, high, vwap, adv120):
    return (
        rank(
            correlation(
...
def alpha_66(open, low, high, vwap):
    return (
...
def alpha_67(high, vwap, indclass, adv20):
    return (
...
def alpha_68(low, high, close, adv15):
    return (
...
def alpha_69(close, vwap, indclass, adv20):
    return (
...
def alpha_70(close, vwap, indclass, adv50):
    return (
...
def alpha_71(open, low, close, vwap, adv180):
    return max(
        ts_rank(
            decay_linear(
...
def alpha_74(high, close, volume, vwap, adv30):
    return (
...
def alpha_77(low, high, vwap, adv40):
    return min(
...
def alpha_78(low, volume, vwap, adv40):
    return rank(
        correlation(
...
def alpha_80(open, high, indclass, adv10):
    return (
        rank(
            sign(
                delta(
...
def alpha_81(volume, vwap, adv10):
    return (
        rank(
            log(
                product(
                    rank((rank(correlation(vwap, sum(adv10, 49.6054), 8.47743)) ^ 4)),
                    14.9655,
                )
            )
        )
...
def alpha_82(open, volume, indclass):
    return (
        min(
...
def alpha_89(low, vwap, indclass, adv10):
    return ts_rank(
        decay_linear(
...
def alpha_90(low, close, indclass, adv40):
    return (
...
def alpha_91(close, volume, vwap, indclass, adv30):
    return (
        ts_rank(
            decay_linear(
                decay_linear(
...
def alpha_92(open, low, high, close, adv30):
    return min(
        ts_rank(
...
def alpha_96(close, volume, vwap, adv60):
    return (
        max(
            ts_rank(
...
def alpha_97(low, vwap, indclass, adv60):
    return (
        rank(
            decay_linear(
                delta(
...
def alpha_100(low, high, close, volume, indclass, adv20):
    return 0 - (
        1
        * (
            (
...
def alpha_101(open, low, high, close):
